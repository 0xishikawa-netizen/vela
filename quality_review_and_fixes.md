...
| M-02 | Low | 保守性 | `src/store/projectStore.ts:100–106` & `src/lib/projectSanitize.ts:19–25` | `ASPECT_RATIOS` が 2 ファイルに同一内容で二重定義 |
...
| M-02 | ASPECT_RATIOS 二重定義 | 設計変更に該当（単一ソースへの統合） | §10 設計変更提案 D-3 を参照 |
...
| M-02 ASPECT_RATIOS 統合 | Low | 設計承認後に実装 |
...
**現状のリスク**: renderer 側のバグや悪意ある拡張によって任意の object が main に届く可能性がある。main 側の `isPayload()` で守られているが、preload での early fail の方がセキュリティ上望ましい。  
**提案**: preload 内で `WhisperLocalStartPayload` 型に近い簡易検証（runId, binaryPath, modelPath, sourceMediaPath の string チェック）を追加し、不正なペイロードは `Promise.reject()` で返す。  
**影響範囲**: `electron/preload.ts` のみ（1 関数）  
**破壊的変更**: なし
...
**概要**: `readAudioFileForWaveform`, `getThumbnail`, `getMediaInfo` などのメディア IPC ハンドラが受け取るファイルパスを main 側で検証していない（ファイルシステム全体が読める）。  
**現状のリスク**: renderer バグ、または将来 webContents が侵害された場合に任意ファイルの読み取りが可能。  
**提案**: `dialog:openMedia` で選択したパスを main が `Set<string>` に保持し、メディア系 IPC ではその whitelist を参照してから `readFile`/`stat` を実行するアーキテクチャに変更する。  
**影響範囲**: `electron/main.ts`, `electron/ipc/media.ts`, IPC のセッション管理  
**破壊的変更**: ダイアログを経由せず直接パスを渡すフローがあれば要調整
...
**現状のリスク**: どちらかを更新し忘れると不整合が生じる（例: 新アスペクト比追加時）。  
**提案**: `src/lib/constants.ts` に `ASPECT_RATIOS` を一元化し、両ファイルから import する。  
**影響範囲**: `projectStore.ts`, `projectSanitize.ts`, `constants.ts`  
**破壊的変更**: なし（内部リファクタ）
...
**提案**: `waveformState: 'idle' | 'loading' | 'failed' | 'ready'` の単一 enum フィールドに統合し、`waveformData` は別途保持する。  
**影響範囲**: `src/store/editorStore.ts`, `usePlayback`, `Preview.tsx` など waveform を参照するすべての箇所  
**破壊的変更**: waveform 状態を読む全コンポーネントの変更が必要
